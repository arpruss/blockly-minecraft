...
Blockly.JavaScript.init = function(workspace) {
  ...
  Blockly.JavaScript.startups_ = Object.create(null);
  Blockly.JavaScript.cleanups_ = Object.create(null);
...
Blockly.JavaScript.finish = function(code) {
  ...
  for (var i = 0 ; i < defNames.length ; i++) {
    definitions.push(Blockly.JavaScript.definitions_[defNames[i]]);
  }
  var cleanNames = [];
  for (var name in Blockly.JavaScript.cleanups_) {
    cleanNames.push(name);
  }
  cleanNames = cleanNames.sort().reverse();
  var cleanups = [];
  for (var i = 0 ; i < cleanNames.length ; i++) {
    cleanups.push(Blockly.JavaScript.cleanups_[cleanNames[i]]);
  }
